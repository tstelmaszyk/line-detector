...
TEST_CASE("LaneMask isole une bande blanche et ignore la route grise unie") {
    cv::Mat bgr(100, 100, CV_8UC3, cv::Scalar(110, 110, 110)); // route grise
    ...
    CHECK(out.at<uchar>(50, 50) == 255); // au centre de la bande blanche
    CHECK(out.at<uchar>(50, 5)  == 0);   // route grise unie -> aucun bord
}
